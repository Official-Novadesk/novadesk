...
namespace
{
    void CaptureTextAnimationFrom(TextElement *text, const Widget::AnimationTarget &to, const Widget::AnimationTarget &from, Widget::AnimationTarget &outFrom)
    {
        if (!text)
            return;
        ...
        if (to.hasFontSize)
        {
            outFrom.hasFontSize = true;
            outFrom.fontSize = from.hasFontSize ? from.fontSize : static_cast<float>(text->GetFontSize());
        }
        if (to.hasFontWeight)
        {
            outFrom.hasFontWeight = true;
            outFrom.fontWeight = from.hasFontWeight ? from.fontWeight : static_cast<float>(text->GetFontWeight());
        }
        if (to.hasLetterSpacing)
        {
            outFrom.hasLetterSpacing = true;
            outFrom.letterSpacing = from.hasLetterSpacing ? from.letterSpacing : text->GetLetterSpacing();
        }
        if (to.hasFontColor)
        {
            outFrom.hasFontColor = true;
            if (from.hasFontColor)
            {
                outFrom.fontColorR = from.fontColorR;
                outFrom.fontColorG = from.fontColorG;
                outFrom.fontColorB = from.fontColorB;
                outFrom.fontAlpha = from.fontAlpha;
            }
            else
            {
                const COLORREF color = text->GetFontColor();
                outFrom.fontColorR = static_cast<float>(GetRValue(color));
                outFrom.fontColorG = static_cast<float>(GetGValue(color));
                outFrom.fontColorB = static_cast<float>(GetBValue(color));
                outFrom.fontAlpha = static_cast<float>(text->GetFontAlpha());
            }
        }
    }
...
void Widget::StartElementAnimation(const std::wstring &id, const AnimationTarget &to, const AnimationTarget &from, int durationMs, const std::wstring &easing, int iterationCount)
{
    ...
    if (to.HasTextProps() && element->GetType() != ELEMENT_TEXT)
        return;
    ...
    anim.from.hasFontSize = to.hasFontSize;
    anim.from.hasFontWeight = to.hasFontWeight;
    anim.from.hasLetterSpacing = to.hasLetterSpacing;
    anim.from.hasFontColor = to.hasFontColor;
    ...
    TextElement *textEl = element->GetType() == ELEMENT_TEXT ? static_cast<TextElement *>(element) : nullptr;
    CaptureTextAnimationFrom(textEl, to, from, anim.from);
    if (textEl && from.HasTextProps())
        ApplyTextAnimationTarget(textEl, from);
...
void Widget::StartElementKeyframeAnimation(const std::wstring &id, const std::vector<AnimationKeyframe> &keyframes, int durationMs, const std::wstring &easing, int iterationCount)
{
    ...
    std::vector<AnimationKeyframe> sorted = keyframes;
    std::sort(sorted.begin(), sorted.end(), [](const AnimationKeyframe &a, const AnimationKeyframe &b)
...
void Widget::StepAnimations()
{
    ...
    for (auto it = m_Animations.begin(); it != m_Animations.end();)
    {
        ...
        else
        {
            ...
            sampled = SampleAnimationTarget(it->from, it->to, p);
        }
