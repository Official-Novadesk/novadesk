    ...
    const BoxBorderPaintParams& params, ID2D1Brush* strokeBrush)
{
    ...
    auto paintRectangularDashedBorder = [&]()
        {
            ...
            auto buildSegments = [&](float length)
                {
                    std::vector<DashSegment> segments;
                    if (length <= 0.0f)
                        return segments;

                    const float targetPeriod = std::max(1.0f, w * 4.0f);
                    const int dashCount = std::max(
                        1,
                        static_cast<int>(std::round((length + w) / targetPeriod)));
                    const float unit = length / static_cast<float>((dashCount * 4) - 1);
                    const float dash = unit * 3.0f;
                    const float gap = unit;

                    for (int i = 0; i < dashCount; ++i)
                    {
                        const float start = static_cast<float>(i) * (dash + gap);
                        const float end = (i == dashCount - 1) ? length : start + dash;
            ...
            auto paintSegments = [&]()
                {
                    for (const auto& segment : buildSegments(R - L))
                    {
                        context->FillRectangle(D2D1::RectF(L + segment.start, T, L + segment.end, T + w), strokeBrush);
                        context->FillRectangle(D2D1::RectF(L + segment.start, B - w, L + segment.end, B), strokeBrush);
                    }
                    for (const auto& segment : buildSegments(B - T))
                    {
                        context->FillRectangle(D2D1::RectF(L, T + segment.start, L + w, T + segment.end), strokeBrush);
                        context->FillRectangle(D2D1::RectF(R - w, T + segment.start, R, T + segment.end), strokeBrush);
                    }
                };

            if (rect.radiusX <= 0.0f && rect.radiusY <= 0.0f)
            {
                paintSegments();
                return;
            }
            ...
            if (!factory || FAILED(factory->CreateRoundedRectangleGeometry(rect, &clipGeometry)))
            {
                paintSegments();
                return;
            }
    ...
    if (sameStyle &&
        ...
        rect.radiusX <= params.strokeWidth &&
        rect.radiusY <= params.strokeWidth)
    {
        paintRectangularDashedBorder();
        return;
    }
    ...
    else
    {
        ...
        const bool hasRadius = rx > w || ry > w;
        ...
        auto drawRectSideDashed = [&](int sideIndex)
            {
                ...
                float dashes[] = { 3.0f, 1.0f };
                if (FAILED(factory->CreateStrokeStyle(props, dashes, 2, &dashedStroke)))
                    return;
                ...
                case 0:
                    context->DrawLine(
                        D2D1::Point2F(L, T + w * 0.5f),
                        D2D1::Point2F(R, T + w * 0.5f),
                        strokeBrush,
                        w,
                        dashedStroke.Get());
                    break;
                case 1:
                    context->DrawLine(
                        D2D1::Point2F(R - w * 0.5f, T),
                        D2D1::Point2F(R - w * 0.5f, B),
                        strokeBrush,
                        w,
                        dashedStroke.Get());
                    break;
                case 2:
                    context->DrawLine(
                        D2D1::Point2F(L, B - w * 0.5f),
                        D2D1::Point2F(R, B - w * 0.5f),
                        strokeBrush,
                        w,
                        dashedStroke.Get());
                    break;
                ...
                default:
                    context->DrawLine(
                        D2D1::Point2F(L + w * 0.5f, T),
                        D2D1::Point2F(L + w * 0.5f, B),
                        strokeBrush,
                        w,
                        dashedStroke.Get());
                    break;
        ...
        for (const auto& side : sides)
        {
            if (!side.clip || side.style == BoxBorder::Style::None || side.style == BoxBorder::Style::Hidden)
                continue;
            const bool useSideClip = !hasRadius && side.style == BoxBorder::Style::Dotted;
            if (useSideClip)
            {
                D2D1_LAYER_PARAMETERS1 params = D2D1::LayerParameters1(
                    D2D1::InfiniteRect(),
